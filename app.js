...
const $=id=>document.getElementById(id); const LS_KEY='offbook.v55'; const LEGACY_KEYS=['offbook.v53','offbook.v51','offbook.v50','offbook.v49','offbook.v47','offbook.v46','offbook.v45.multi','screenplayTrainer.v44.multi','screenplayTrainer.v43','screenplayTrainer.v42'];
const state={mode:'script',raw:'',title:'Untitled Script',author:'',lines:[],characters:[],selectedRoles:new Set(),statuses:{},trouble:{},bookmarks:{},misses:[],currentList:[],pos:0,revealed:false,listening:false,recognition:null,selectedFile:null,finalTimer:null,session:{practiced:0,passedFirst:0,retries:0,failed:0},randomQueue:[],blockingRevealed:false};
...
function isLyricSection(t){return /^\s*(?:\[(.+)\]|(VERSE|CHORUS|PRE[- ]?CHORUS|REFRAIN|BRIDGE|INTRO|OUTRO|CODA|HOOK|TAG|INTERLUDE|BREAK|ENDING)(?:\s+\d+)?\s*:?)\s*$/i.test(t||'')}
function lyricSectionName(t){let m=(t||'').trim().match(/^\[(.+)\]$/); if(m)return m[1].trim(); return (t||'').replace(/:$/,'').trim()}
function parseLyrics(raw){
  let warnings=[], title='Untitled Song', author='', lines=[], section='Song';
  raw=(raw||'').replace(/\r/g,'').replace(/\u000c/g,'\n').replace(/\ufffe/g,'\n');
  const rawLines=raw.split('\n');
  rawLines.forEach((line,idx)=>{
    let t=line.trim(); if(!t)return;
    let tm=t.match(/^(?:TITLE|SONG)\s*:\s*(.+)$/i); if(tm){title=tm[1].trim(); return;}
    let am=t.match(/^(?:AUTHOR|BY|COMPOSER|LYRICS)\s*:\s*(.+)$/i); if(am){if(!author)author=am[1].trim(); return;}
    if(isLyricSection(t)){section=lyricSectionName(t)||'Song'; return;}
    if(/^\d+$/.test(t))return;
    let tags=extractTags(t), cleanRaw=tags.text.trim(), speakText=stripPause(stripParen(cleanRaw)).replace(/\s+/g,' ').trim();
    if(!speakText)return;
    let id='L'+String(lines.length+1).padStart(4,'0');
    lines.push({id,globalN:lines.length+1,speaker:'VOCAL',rawText:t,cleanRaw,speakText,pause:pauseTotal(t),block:[],note:tags.note,prop:[],roleN:lines.length+1,section});
  });
  if(!lines.length)warnings.push('No lyric phrases found. Put each phrase on its own line. Section headings such as [Verse 1], CHORUS, or BRIDGE are optional.');
  return {title,author,lines,characters:lines.length?['VOCAL']:[],warnings,counts:{VOCAL:lines.length}};
}
function updateModeUI(){
  const importMode=$('contentMode')?.value||state.mode||'script';
  if($('importFormatHelp')) $('importFormatHelp').textContent=importMode==='lyrics'?'Put each lyric phrase on its own line. Optional section headings: [Verse 1], CHORUS, BRIDGE, etc. The previous phrase becomes your cue.':'Character name on its own line, dialogue below, blank line between speeches.';
  if($('pasteBox')) $('pasteBox').placeholder=importMode==='lyrics'?'TITLE: Audition Song\nAUTHOR: Composer / Lyricist\n\n[Verse 1]\nFirst lyric phrase\nNext lyric phrase\n\n[Chorus]\nChorus phrase one\nChorus phrase two':'TITLE: Script Name\nAUTHOR: Author\n\nALICE\nWhere is Bob?\n\nBOB\nRight here.';
  const lyrics=state.mode==='lyrics';
  if($('practiceRoleHeading')) $('practiceRoleHeading').textContent=lyrics?'3. Lyrics practice':'3. Choose practice roles';
  if($('practiceRoleHelp')) $('practiceRoleHelp').textContent=lyrics?'All imported lyric phrases are included. Each phrase is rehearsed from the phrase immediately before it.':'Select one or more roles you want to practice. Every other character becomes a reader/cue role.';
  if($('roleControls')) $('roleControls').classList.toggle('hide',lyrics);
  if($('startBtn')) $('startBtn').textContent=lyrics?'Start lyrics rehearsal':'Start rehearsal';
  if($('answerBox')) $('answerBox').placeholder=lyrics?'Type or speak the lyric phrase here':'Type or speak your line here';
  if($('revealBtn')) $('revealBtn').textContent=state.revealed?(lyrics?'Hide phrase':'Hide line'):(lyrics?'Reveal phrase':'Reveal line');
}
function importRaw(raw,source){let mode=$('contentMode')?.value||'script'; let p=mode==='lyrics'?parseLyrics(raw):parseScript(raw); state.mode=mode; state.raw=raw; state.title=p.title; state.author=p.author; state.lines=p.lines; state.characters=p.characters; state.selectedRoles=new Set(mode==='lyrics'?(p.lines.length?['VOCAL']:[]):(state.characters.length?[state.characters[0]]:[])); state.currentList=[]; state.pos=0; state.revealed=false; updateModeUI(); saveAll(); renderImport(p.warnings,source); buildPracticeList(); setTab('library');}
function saveAll(){localStorage.setItem(LS_KEY,JSON.stringify({mode:state.mode,raw:state.raw,title:state.title,author:state.author,lines:state.lines,characters:state.characters,selectedRoles:[...state.selectedRoles],statuses:state.statuses,trouble:state.trouble,bookmarks:state.bookmarks,misses:state.misses}))}
function continueSaved(){let s=localStorage.getItem(LS_KEY); if(!s){for(const k of LEGACY_KEYS){s=localStorage.getItem(k); if(s){try{localStorage.setItem(LS_KEY,s); $('saveStatus').textContent='Migrated saved script to OffBook.'}catch(e){} break;}}} if(!s)return; try{let d=JSON.parse(s); Object.assign(state,d); state.mode=d.mode||'script'; if($('contentMode'))$('contentMode').value=state.mode; state.selectedRoles=new Set(d.selectedRoles||[]); if(state.mode==='lyrics'&&state.lines.length)state.selectedRoles=new Set(['VOCAL']); updateModeUI(); state.session={practiced:0,passedFirst:0,retries:0,failed:0}; renderImport([], 'Saved script'); buildPracticeList();}catch(e){console.log(e)}}
function renderImport(warnings=[],source=''){let counts={}; state.lines.forEach(l=>counts[l.speaker]=(counts[l.speaker]||0)+1); let lyrics=state.mode==='lyrics'; $('saveStatus').textContent=state.lines.length?state.title:'No script loaded'; $('importSummary').innerHTML=state.lines.length?`<b>${escapeHtml(state.title)}</b>${state.author?' by '+escapeHtml(state.author):''}<br><span class="subtle">Loaded from ${escapeHtml(source)}. ${lyrics?'Lyrics Mode is ready to rehearse.':'Select one or more roles below.'}</span>`:'Import a script or song to see practice units.'; let sections=lyrics?new Set(state.lines.map(l=>l.section||'Song')).size:0; $('importStats').innerHTML=lyrics?stat('Sections',sections)+stat('Phrases',state.lines.length)+stat('Mode','Lyrics')+stat('Warnings',warnings.length):stat('Characters',state.characters.length)+stat('Dialogue',state.lines.length)+stat('Practice roles',state.selectedRoles.size)+stat('Warnings',warnings.length); $('warnings').textContent=warnings.length?warnings.join('\n'):'No parser warnings.'; updateModeUI(); renderRoleCards(); renderCharacterStats(); renderReview();}
function renderRoleCards(){if(state.mode==='lyrics'){$('roleCards').innerHTML=state.lines.length?`<div class="roleCard active"><span class="roleCheck">♪</span><span><b>Lead vocal</b><br><small class="subtle">${state.lines.length} lyric phrases • practicing all</small></span></div>`:'<p class="subtle">No lyric phrases found.</p>'; return;} let counts={}; state.lines.forEach(l=>counts[l.speaker]=(counts[l.speaker]||0)+1); $('roleCards').innerHTML=state.characters.map(c=>`<button type="button" class="roleCard ${state.selectedRoles.has(c)?'active':''}" data-role="${escapeHtml(c)}"><span class="roleCheck">${state.selectedRoles.has(c)?'✓':'□'}</span><span><b>${escapeHtml(c)}</b><br><small class="subtle">${counts[c]||0} lines ${state.selectedRoles.has(c)?'• practicing':'• cue role'}</small></span></button>`).join('')||'<p class="subtle">No characters found.</p>'; document.querySelectorAll('.roleCard[data-role]').forEach(card=>card.onclick=()=>{let r=card.dataset.role; state.selectedRoles.has(r)?state.selectedRoles.delete(r):state.selectedRoles.add(r); saveAll(); renderRoleCards(); buildPracticeList(); renderImport([], 'Current script');});}
function renderCharacterStats(){let lyrics=state.mode==='lyrics'; if(lyrics){let passed=state.lines.filter(l=>state.statuses[l.id]==='passed').length, needs=state.lines.filter(l=>state.statuses[l.id]==='needs').length, failed=state.lines.filter(l=>state.statuses[l.id]==='failed').length; let sections=new Set(state.lines.map(l=>l.section||'Song')).size; let rows=`<div class="stat" style="text-align:left"><b>Lyrics</b><span class="subtle">${state.lines.length} phrases • ${sections} sections</span><br><span class="badge good">${passed} passed</span><span class="badge warn">${needs} needs</span><span class="badge bad">${failed} failed</span></div>`; $('characterStats').innerHTML=rows; $('reviewCharacterStats').innerHTML=rows; return;} let rows=state.characters.map(c=>{let arr=state.lines.filter(l=>l.speaker===c); let passed=arr.filter(l=>state.statuses[l.id]==='passed').length; let needs=arr.filter(l=>state.statuses[l.id]==='needs').length; let failed=arr.filter(l=>state.statuses[l.id]==='failed').length; let selected=state.selectedRoles.has(c); return `<div class="stat" style="text-align:left"><b>${escapeHtml(c)}</b><span class="subtle">${arr.length} lines ${selected?'• practice':'• cue'}</span><br><span class="badge good">${passed} passed</span><span class="badge warn">${needs} needs</span><span class="badge bad">${failed} failed</span></div>`}).join(''); $('characterStats').innerHTML=rows||'<p class="subtle">No character stats yet.</p>'; $('reviewCharacterStats').innerHTML=rows||'<p class="subtle">No character stats yet.</p>';}
...
function cueFor(line){let idx=state.lines.findIndex(l=>l.id===line.id); if(idx<=0)return []; let mode=$('cueMode')?.value||'last1'; let prev=[]; for(let i=idx-1;i>=0;i--){let l=state.lines[i]; if(mode==='full'){if(state.mode==='lyrics'){if((l.section||'Song')!==(line.section||'Song'))break; prev.unshift(l);}else{if(state.selectedRoles.has(l.speaker))break; prev.unshift(l);}} else {prev.unshift(l); if(prev.length>=(mode==='last2'?2:1))break;} } return prev;}
...
function renderCue(line){let cues=cueFor(line); if(!cues.length)return `<span class="subtle">No cue before this ${state.mode==='lyrics'?'phrase':'line'}.</span>`; return cues.map(c=>state.mode==='lyrics'?`<div class="cueBlock"><b>${escapeHtml(c.section||'Previous phrase')}</b><br>${escapeHtml(c.speakText)}</div>`:`<div class="cueBlock"><b>${escapeHtml(c.speaker)}</b><br>${escapeHtml(c.speakText)}</div>`).join('')}
function renderCurrent(){let c=current(); let lyrics=state.mode==='lyrics'; if(!c){$('currentTitle').textContent=state.title||'No script loaded'; $('currentMeta').textContent='No practice cards. Import material and start rehearsal.'; updateModeUI(); return} $('currentTitle').textContent=state.title; $('currentMeta').textContent=lyrics?`Card ${state.pos+1} of ${state.currentList.length} • Phrase ${c.globalN} • ${c.section||'Song'}${c.rep?` • repeat ${c.rep}`:''}`:`Card ${state.pos+1} of ${state.currentList.length} • ${c.speaker} line ${c.roleN} • overall ${c.globalN}${c.rep?` • repeat ${c.rep}`:''}`; $('currentBadges').innerHTML=statusBadge(c)+(state.bookmarks[c.id]?'<span class="badge mark">★</span>':''); $('cueText').innerHTML=renderCue(c); $('speakerLabel').textContent=lyrics?`${c.section||'Song'} • Phrase ${c.globalN}`:`${c.speaker} • Line ${c.roleN}`; $('targetLine').textContent=state.revealed?c.speakText:(lyrics?'Hidden lyric phrase':'Hidden blurred text'); $('targetLine').classList.toggle('hiddenLine',!state.revealed); $('revealBtn').textContent=state.revealed?(lyrics?'Hide phrase':'Hide line'):(lyrics?'Reveal phrase':'Reveal line'); let extras=renderLineExtras(c); $('lineExtras').innerHTML=extras; $('lineExtras').classList.toggle('hide',!extras); $('toggleBlockingBtn').textContent=state.blockingRevealed?'Hide blocking':'Reveal blocking'; $('answerBox').value=''; $('checkResult').textContent=''; updateModeUI(); renderReview();}
...
function startRehearsal(){if(state.mode==='lyrics'&&state.lines.length)state.selectedRoles=new Set(['VOCAL']); if(!state.selectedRoles.size){alert('Pick at least one practice role first.');return} buildPracticeList(); if(!state.currentList.length){alert('No lines in this pool.');return} state.pos=0; state.revealed=false; state.blockingRevealed=false; setTab('rehearse'); renderCurrent()}
...
if($('contentMode')) $('contentMode').onchange=()=>updateModeUI(); updateModeUI();
...
if($('enableSoundBtn')) $('enableSoundBtn').onclick=async()=>{if(await enableSound()) rawTone(520,0.10,0,'sine',0.22)}; if($('testDingBtn')) $('testDingBtn').onclick=()=>playPassSound(true); if($('testErrBtn')) $('testErrBtn').onclick=()=>playFailSound(true); if($('cueModeQuick')) $('cueModeQuick').onchange=()=>{if($('cueModeQuick').value!=='same'){$('cueMode').value=$('cueModeQuick').value; renderCurrent();}};
$('loadPasteBtn').onclick=()=>importRaw($('pasteBox').value,'Pasted text'); $('clearPasteBtn').onclick=()=>$('pasteBox').value=''; $('continueBtn').onclick=continueSaved; $('clearSavedBtn').onclick=()=>{localStorage.removeItem(LS_KEY); LEGACY_KEYS.forEach(k=>localStorage.removeItem(k)); alert('Saved OffBook material cleared.')}; $('downloadTemplateBtn').onclick=()=>{let lyrics=($('contentMode')?.value||'script')==='lyrics'; downloadText(lyrics?'offbook_lyrics_template.txt':'offbook_script_template.txt',lyrics?'TITLE: Audition Song\nAUTHOR: Composer / Lyricist\n\n[Verse 1]\nFirst lyric phrase\nSecond lyric phrase\n\n[Chorus]\nFirst chorus phrase\nSecond chorus phrase\n':'TITLE: Script Name\nAUTHOR: Author Name\n\nALICE\nWhere is Bob?\n\nBOB\nRight here.\n\nCAROL\nCan we get started?\n\nALICE\nOne second.\n[pause:3]\nReady.\n')};
...
if('serviceWorker'in navigator){navigator.serviceWorker.register('sw.js?v=55').then(reg=>{reg.update&&reg.update();}).catch(()=>{})}
continueSaved();
